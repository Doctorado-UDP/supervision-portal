...
export default async function AdminDashboardPage() {
  ...
  const thirtyDaysString = thirtyDaysFromNow
    .toISOString()
    .slice(0, 10);
  ...
  const [
    ...
    supabase
      ...
      .gte("target_date", todayString)
      .lte("target_date", thirtyDaysString)
  ...
  const submissionIdsWithFeedback = new Set(
  ...
  const awaitingFeedback = (
    ...
    (submission) =>
      !submissionIdsWithFeedback.has(submission.id)
  ).length;

  const upcomingDeadlines = milestonesResult.count ?? 0;

  const usedStorageBytes = Number(
    storageResult.data?.total_bytes ?? 0
  );

  const usedStorageMb =
    formatMegabytes(usedStorageBytes);

  const storagePercentage =
    STORAGE_LIMIT_BYTES === 0
      ? 0
      : Math.min(
          (usedStorageBytes / STORAGE_LIMIT_BYTES) * 100,
          100
        );
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="mt-2 text-gray-600">
          Overview of supervision activity and upcoming work.
        </p>
      </div>
      ...
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <MetricCard
          ...
          description="Submissions without supervisor feedback"
        />
        ...
        <MetricCard
          title="Stored files"
          value={Number(
            storageResult.data?.total_files ?? 0
          )}
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-gray-500">
                Next five incomplete student deadlines.
              </p>
            </div>
            ...
            {upcomingMilestones.length === 0 ? (
              ...
              <div className="divide-y divide-gray-100">
                {upcomingMilestones.map((milestone) => {
                  const userId = studentUserMap.get(
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
          ...
          <div className="mt-6">
            ...
            <div className="mt-4 flex justify-between text-sm text-gray-500">
              <span>Status: {storageStatus}</span>
              <span>1 GB limit</span>
            </div>

            {storagePercentage >= 85 && (
              <p className="mt-4 rounded-md bg-red-50 px-3 py-2 text-sm text-red-700">
                Storage usage is above 85%. Consider
                migrating document storage or upgrading
                capacity.
              </p>
            )}
            ...
            {storagePercentage >= 70 &&
              storagePercentage < 85 && (
                <p className="mt-4 rounded-md bg-amber-50 px-3 py-2 text-sm text-amber-700">
                  Storage usage has exceeded the 70%
      ...
      <section className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
        ...
        <p className="mt-1 text-sm text-gray-500">
          Manage student records, submissions and supervision progress.
        </p>
