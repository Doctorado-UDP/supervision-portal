...
def section_replace(text: str, section: str, pattern: str, replacement: str, label: str) -> str:
    ...
    block = match.group(1)
    new_block, count = re.subn(pattern, replacement, block, count=1, flags=re.MULTILINE)
    if count != 1:
...
config = section_replace(config, "db.seed", r"^enabled = true$", "enabled = false", "seed config")
config = section_replace(config, "storage", r'^file_size_limit = "50MiB"$', 'file_size_limit = "25MiB"', "storage limit")
config = section_replace(
    config,
    "auth",
    r'^site_url = ".*"$',
    'site_url = "https://supervision.bgonzalezbustamante.com"',
    "auth site url",
)
config = section_replace(
    config,
...
config = section_replace(config, "auth.rate_limit", r"^email_sent = \d+$", "email_sent = 30", "email rate limit")
config = section_replace(config, "auth.email", r'^max_frequency = ".*"$', 'max_frequency = "1m"', "email frequency")
config_path.write_text(config)

changelog_path = Path("CHANGELOG.md")
changelog = changelog_path.read_text()
heading = '## v0.1.0-beta.4 "Ivory Monolith"'
if heading not in changelog:
    entry = '''## v0.1.0-beta.4 "Ivory Monolith"
